...
A namespace `tenfold.mini-reducers` lives in `src/tenfold/mini_reducers.clj`.
...
Type `(map inc [1 2 3]) (println "done")` on one line.
...
Put the cursor in a `defn` and press <kbd>ctrl+alt+c</kbd> <kbd>i</kbd> to **instrument** it.
Calva places invisible breakpoints at each logical step inside the function.
Call the function and execution pauses at the first one, with VS Code's debugger controls (continue, step over, step in, step out) driving it from there.
